...
rotate::	movei #ANGLE,r14	; fetch cosine and sine
...
;movei #max_y<<16|max_x,dummy
;store dummy,(blitter+$08)
...
.loop1		load (pptr),point
		...
		UNREG point,EDGE,LOOP,GOON,x1,x2,y1,y2
****************
* edge (x1,y1)-(x2,y2)
* Bresenham-Algo.
****************
...
***************
		...
		jump z,(dummy)
		nop
.loop001	cmpq #0,delta
.loop002	jr nn,.cont04
		add d_x,x1
		subq #1,step
		jr nn,.loop001
		add delta_y,delta
...
.cont04		add delta_y,delta
		...
		jr n,.loop002
		cmpq #0,delta
.positiv0	sub y1,y_count
		jump n,(r13)		    ;.exit
		movefa x_save.a,ptr
		jump z,(r13)		    ;.exit
		shlq #2,y1		      ; als ptr fr x-save
		add y1,ptr
